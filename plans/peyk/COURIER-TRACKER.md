...
last: s8 claimed PK4.1 (99cb57e code, e00cad1 seam fix + rig, 64c2dc6 evidence .conductor/evidence/PK4/pk4.1.md, rig 16/16). PK3 all claimed by s6/s7.
  Built: Conductor.Core/Courier/Room.cs - Room {project, repo?, chats{admin,observer}, footer{counters,live,pending}, voice path} at <state home>/courier/rooms/<slug>.json (slug = lowercased name, non-alnum -> '-'). Rooms.List/Read/Save/Find(repo: by repo, then by folder name)/Resolve(repo, planChats pairs, CourierSettings) = D6 fallback plan chats -> courier chats (an ambiguous courier profile is null). RoomImport.Offers/Import/Parse reads <dir>/*/config.json (chats.*, report.counters/footerLive/footerPending), voice = path of voice.md, never overwrites.
  Verb: `room list|show|add|import` (RoomCommand.Run(settings, TextWriter, stateHomeRoot) is the seam); registered in Program.cs + CompletionCommand.Verbs; cli.md "## Rooms" section + operating.md s2 row. Output never prints a chat id. docs/rooms/character.md is in the tree (report.ps1 and in-repo voice pointers rewritten); the skill's own copy is untouched.
  The owner's real courier home has NO rooms yet: the proof imported into a scratch state home. The real import is one command for the owner (`conductor room import`) after the reinstall - no session runs it.
  Trap: KS11_1SeamBoundaryTests is a two-way ratchet - any new Core file naming a Telegram* type fails; pass plan.Telegram.ResolvedChats() pairs from the adapter side. powershell.exe launched from pwsh cannot see Get-FileHash (inherited PSModulePath) - use .NET SHA256.
  Rigs: tools/peyk/pk3-rig-lib.ps1 (stub, scratch courier, Invoke-Engine, scratch run + fake agent); tools/peyk/pk4-1-rooms-proof.ps1. A run's log is .conductor/logs/conductor-<date>.log.
  PK2 (still true): real 'Conductor Courier' ARMED 2026-09-17T11:37:30Z, speaks protocol 2; bug #93 stays OPEN for PK6.1 (`tools/peyk/pk2-3-arm-real-courier.ps1 -ReadOnly`; first death record is the installer's /End).
next: PK4.2 (the card: task --done --tell, composed at the verdict, Rooms.Resolve for the observer chat), then PK4.3. Do not restart, stop or reinstall the armed courier.
