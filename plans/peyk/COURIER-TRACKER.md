...
last: s8 claimed PK4.1 (99cb57e, e00cad1, 80743dd; evidence pk4.1.md, rig 16/16) and PK4.2 (c97b8c5 --tell, b636bba card, 1a90426 rig + flush fix; evidence pk4.2.md, rig 3/3, neighbours 596/596).
  PK4.1: Room/RoomChats/RoomFooter (Courier/Room.cs) + Rooms (Courier/Rooms.cs: List/Read/Save/Find by repo then folder name/Resolve = D6 fallback plan chats -> courier chats) + RoomImport; `room list|show|add|import`; docs/rooms/character.md. The owner's REAL courier home has no rooms yet - the proof imported into a scratch home; `conductor room import` is the owner's one command after the reinstall.
  PK4.2: TaskStatusChanged.Tell -> TaskItem.Tell (CardWords refuses at the claim). VerdictEngine.ConfirmPendingCheckpoints queues ids (VerdictEngine.Cards.cs); drained in RunLoop.Evidence RegisterEvidenceAsync and at ConfirmStageAsync, which also posts ONE stage card (first confirmation only). IRunNotifier.PushCheckpointCardAsync/PushStageCardAsync (defaulted); TelegramService.Channel.cs resolves the room and sends like say (courier, else direct). CardFacts = bar/counts/live rule (stages[].deploys)/pair (*before, *after visual artifacts). Templates checkpoint-card, stage-card in NotifyDefaults. HeldWordsBattery (own file) always on.
  Facts measured: a stage is CONFIRMED only under gatePolicy perPhase. The composer must FlushEvents before reading (evidence persists via async drain). ArchitectureTests: 3 types per file, RunLoop.cs AT 500 lines - run them after adding files. KS11_1SeamBoundary: no new Core file may name Telegram* (use _cfg in the adapter files only).
  Rigs: tests HarnessTests.Cards.cs (real RunCommand, fake agent claims via test-bin conductor.exe, RecordingBotApi); tools/peyk/pk4-1-rooms-proof.ps1; tools/peyk/pk3-rig-lib.ps1.
...
next: PK4.3 - RoomVoiceBattery (character.md + the room's voice path, byte cap, on when the room has an observer chat), report.ps1 deleted from the skill, the three field plans' report.ps1 rule rewritten to --tell (plan files only, commit nothing there), SF7_1 pins --tell in cli.md. Do not restart, stop or reinstall the armed courier.
