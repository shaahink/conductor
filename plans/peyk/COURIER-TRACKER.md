...
PK1.2 IN PROGRESS (5605f8b engine side, e775bbe installer): measured that a courier BESIDE the engine locks shared dlls (evidence pk1.2-lock-measurement.log), so the courier installs to <install>\courier; CourierBinary.Resolve/ShapeOf; `courier install` defaults to it with --task-name; preflight courier line is shape-aware + --courier-task; install.ps1 leaves an own-directory courier running, moves a pre-D1 one once, -CourierOnly [-NoCourierStart] never publishes the engine (registers via this tree's engine build).
next: run tools/peyk/pk1-2-live-proof.ps1 (scratch install under TEMP, scratch task never started), copy its log to .conductor/evidence/PK1/pk1.2-live-proof.log, git add -f, claim PK1.2. At PK2.3 the real task still runs v0.5.0 `conductor.exe courier run` - -CourierOnly stops it via /End, which is the migration.
