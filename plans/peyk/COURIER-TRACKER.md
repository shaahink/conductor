...
last: s6 CLAIMED PK3.2 (code 965a6dd, rig a9c6a70 20/20, evidence da2626f .conductor/evidence/PK3/pk3.2.md; real send id 3821 deleted): `conductor say` (src/Conductor/Commands/SayCommand.cs, RunAsync has test seams); CourierChannel `direct:` = TelegramService.SendAsync when no courier takes a push (CourierAck.Unanswered = connection refused only, never a timeout), logs `courier unreachable - sent directly`; health line `last push went directly|through the courier at T` via CourierChannel.LastDeliveryFor(stateHome). Bug #98 (caption 1025) closed. Rig helpers: tools/peyk/pk3-rig-lib.ps1 (stub, relay, scratch courier, Invoke-Engine, scratch run with fake agent in pk3-2 part B). A run's log is .conductor/logs/conductor-<date>.log.
  s6 CLAIMED PK3.1 (code 0d43747, evidence 04a58b0 .conductor/evidence/PK3/pk3.1.md, rig 23/23, tests 280/280). CourierProtocol.Version=3; loopback POST /send|/react|/delete, GET /chats via ICourierDesk/CourierDesk (core) -> ICourierSource; ids on CourierAck.MessageIds; <home>/courier/messages.jsonl (CourierMessageLedger). /push (2) still accepted, now uploads files (bug #76 closed). Real send: admin DM, message id 3820, deleted.
...
next: PK3.3 (the run's /hello registers its project, D4). Do not restart, stop or reinstall the armed courier.
