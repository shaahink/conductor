...
2. **Reproduce each failure** with the narrowest command that shows it — the single failing test, the
   one invocation that trips it. Do NOT re-run the whole battery; conductor re-runs it after you exit. Anything longer than a few minutes runs under `conductor bg`, and you wait for it INSIDE a tool
   call — never by ending your turn, which ends the process.
3. **Suspect the environment before the code when the shape fits:** a red that comes with a much
   ...
   stale artifact, a locked DLL (`dotnet build-server shutdown`), an orphaned test host, or a scratch
   courier still holding a port — this repo's known flakes. Check that before you theorise about a
   defect.
4. **On a boundary change, read the architecture test first.** If the red is an architecture assertion,
...
7. **Close** as any session does: evidence artifact, claim what is genuinely done with
   ...
   block in {tracker}, commit (never prefixed `chore(conductor):`), push the plan's branch (`branchPattern` in the plan file;
   `git branch --show-current` must match it).
