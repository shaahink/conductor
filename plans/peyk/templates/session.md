...
this repo's test suite has harness fixtures (HarnessTests, fake-agent tooling, the courier rigs under
tests/) built for exactly that; prefer extending them over asserting from source reading. Two hard rules
when you drive anything live: exercise YOUR code through the fresh build
...
plan, its own state dir, its own courier home and its own courier port, never this repo's and never the
real courier's. See the traps below.

## The design doc is the spec, and its decisions are taken

Your stage notes name the decisions (D-numbers) and the stage table of `docs/dev/NEXT-ERA-FINDINGS-2026-09-17.md`
that specify your checkpoints. Read that stage's row and those decisions — not the whole document, and
not Appendix A unless your checkpoint names it. A decision states its rejected alternative on purpose:
build the decision. If it is wrong by measurement, the measurement goes in the handoff and the session
stops there; that is a finding worth more than the checkpoint.
...
- Long commands (full test suite, golden regeneration, a rig that waits) run under `conductor bg` — a
  silent foreground command reads as a stall and gets you killed.
- **A wait happens inside a tool call** — a status loop with a sub-timeout that prints a line per minute —
  never by ending your turn. In `claude -p` the end of the turn is the end of the process: a session that
  armed a battery and "waited for the verdict" lost the battery and its handoff. Write the handoff BEFORE
  you start anything long.

## Do, in order

1. **ORIENT, then say what you are taking.** Read the `## Handoff` block in {tracker}, run
   `conductor task --list` for your stage's rows, and read YOUR stage's row and decisions in the spec
   named in the stage notes below — nothing else. Then mark what you are starting:
...
2. **DECLARE ACCEPTANCE, in writing, before you edit anything.** One line per checkpoint: what must be
   true for it to be done, and what artifact will show it — the stage table's acceptance column is the
   floor, not the ceiling. `conductor note` it.
...
4. **CLOSE.** Produce the evidence artifact under `.conductor/evidence/<stage>/`, then:
   ...
   **Run it before you write the handoff**, so a session that runs out of room still lands the claim.
   ...
   commit per checkpoint with the repo's trailer convention, and push the plan's branch — the one
   `branchPattern` in the plan file names; `git branch --show-current` must match it before you push.
...
- **Measure a verdict; do not read it off a doc comment.** This project has been wrong about its own
  ...
  the courier that "restarts on failure". Check what the code DOES, and cite the file and line.
...
- **A real Telegram message is rationed to one per proof, to the admin DM, then deleted**, with both
  ids in the evidence. The room files under the owner's home are pointed at, never copied.
- If genuinely blocked on a decision only the owner can make, add an escalation line to the handoff
  ...
  push, and end the session. A decision the plan doc already takes is never one of these.
- **Never prefix your own commits `chore(conductor):`** — that is the engine's bookkeeping prefix, and the
  stage squash collapses consecutive ones after you have pushed.
