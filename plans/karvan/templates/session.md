...
4. **CLOSE.** Produce the evidence artifact under `.conductor/evidence/<stage>/`, then:
   ...
   **Run it before you write the handoff**, so a session that runs out of room still lands the claim.
   ...
   commit per checkpoint with the repo's trailer convention, and push `feat/karvan`.
...
- **Refactor stages keep the suite green at every step.** 1,547 test methods are the net. If a move
  makes a test unbuildable, that is a signal about the boundary, not a licence to delete the test.
