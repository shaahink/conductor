...
$env:CONDUCTOR_PLAN = ''
Write-Host "===== PREFLIGHT (compose leg + verdict) ====="
& $exe preflight -p (Join-Path $repo 'fixture.plan.json') --no-auth-check --no-update-check 2>&1 |
  Select-String "compose|READY" | Select-Object -First 4
Write-Host "preflight exit=$LASTEXITCODE"

Write-Host "===== LIVE RUN --once (full log to file) ====="
...
Get-Content (Join-Path $rig 'live.log') | Select-String "session #|queuing a fix|completion NOT|plan .* complete" |
...
Get-Content $statePath | Select-String 'status|sessionCounter|attemptsThisStage|pendingFix' | ForEach-Object { $_.Line }
