...
$rj = $repo.Replace('\', '\\')
$plan = '{ "name":"ks34-phase","repo":"' + $rj + '","tracker":"TRACKER.md","gatePolicy":"perPhase",' +
        '"agent":{"command":"git","args":["-p","{prompt}"]},' +
        '"audit":{"enabled":true,"enableParallel":false},' +
...
{
  "planName": "ks34-phase",
  "runId": "ph000000000000000000000000000001",
  "status": "Idle",
  "currentStage": "S1",
  "sessionCounter": 1,
  "attemptsThisStage": 0,
...
Get-Content (Join-Path $rig 'live.log') | Select-String "session #|scheduling|CONFIRMED|audit" |
