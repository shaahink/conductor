...
public sealed class DoctorCommandTests : IDisposable
{
    ...
    {
        var check = DoctorCommand.CheckBudget(Plan(), currentCostUsd: 999m, hasRun: true, budgetGrantUsd: 0m, budgetGrantTokens: 0L);
        Assert.Equal("warn", check.State);
    ...
    public void CheckBudget_Ok_WhenNoRunYet()
    {
        ...
        var check = DoctorCommand.CheckBudget(plan, currentCostUsd: 0m, hasRun: false, budgetGrantUsd: 0m, budgetGrantTokens: 0L);
        Assert.Equal("ok", check.State);
    }
    ...
    {
        var plan = Plan(p => p.Limits.MaxRunCostUsd = 10m);
        var check = DoctorCommand.CheckBudget(plan, currentCostUsd: 9m, hasRun: true, budgetGrantUsd: 0m, budgetGrantTokens: 0L); // 90%
        Assert.Equal("warn", check.State);
    }
    ...
    {
        var plan = Plan(p => p.Limits.MaxRunCostUsd = 10m);
        var check = DoctorCommand.CheckBudget(plan, currentCostUsd: 10m, hasRun: true, budgetGrantUsd: 0m, budgetGrantTokens: 0L);
        Assert.Equal("fail", check.State);
    }
