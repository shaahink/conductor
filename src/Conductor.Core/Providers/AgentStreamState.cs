    ...
    Action<ToolRefusal>? onRefusal = null)
{
    ...
    public void EmitTokenDelta(long input, long output, long reasoning, long cacheRead, decimal costUsd)
        => _onTokenDelta?.Invoke(input, output, reasoning, cacheRead, 0, costUsd, 0);
    ...
    public void EmitTokenDelta(long input, long output, long reasoning, long cacheRead, long cacheWrite, decimal costUsd)
        => _onTokenDelta?.Invoke(input, output, reasoning, cacheRead, cacheWrite, costUsd, 0);

    /// <summary>Bug #53 — the same delta, with the one-hour part of <paramref name="cacheWrite"/>
    /// named too. <paramref name="cacheWrite1h"/> is a SUBSET of <paramref name="cacheWrite"/>.</summary>
    public void EmitTokenDelta(long input, long output, long reasoning, long cacheRead, long cacheWrite, long cacheWrite1h, decimal costUsd)
        => _onTokenDelta?.Invoke(input, output, reasoning, cacheRead, cacheWrite, costUsd, cacheWrite1h);
    ...
    /// <summary>Bug #53 — of <see cref="TokensCacheWrite"/>, how many were written with the ONE-HOUR
    /// TTL (<c>usage.cache_creation.ephemeral_1h_input_tokens</c>). A subset of a subset, kept by the
    /// same rule KS7.3 kept the write half by: named, never moved. The wire splits cache creation by
    /// TTL because the two are billed differently (1.25x and 2x the fresh rate), and a rate-based cost
    /// model that priced every write at one rate would misprice exactly the half that costs more.
    /// Null when the wire reported no split — an older CLI, another provider — which a consumer must
    /// read as "not reported", never as "all five-minute".</summary>
    public long? TokensCacheWrite1h { get; set; }
}
...
/// (it is contained IN <paramref name="input"/>). Bug #53 adds <paramref name="cacheWrite1h"/>, the
/// one-hour part of <paramref name="cacheWrite"/> — contained in it the same way.</summary>
public delegate void TokenDeltaSink(
    long input, long output, long reasoning, long cacheRead, long cacheWrite, decimal costUsd, long cacheWrite1h);
