...
/// <para>Partial: the GitHub mirror's attach/poke/drain surface lives in
/// <c>RunContext.Mirror.cs</c>, because a one-way mirror to a destination outside the run is a
/// different job from the run's own state, budget and logging.</para>
/// </summary>
public sealed partial class RunContext
{
    ...
    /// <summary>SF4.2 / DV1.2 — a queue item that arrives while the owner is away from the keyboard
    /// reaches them. That is the entire point of the surface: <c>.conductor/OWNER-QUEUE.md</c> and
    /// <c>GET /owner/queue</c> both require someone to be LOOKING, and the case this was written for
    /// is the one where nobody is.
    ...
    /// obligation, not twenty times, and a boundary that changed nothing sends nothing at all.</para>
    /// <para>DV1.2 moved the composition out of this method. It used to build one batched message
    /// here with a StringBuilder: three obligations arrived as one notification, which reads as one
    /// thing to deal with later, and it was the only push in the engine that was not an owner-editable
    /// template. Now the surface sends one message per item in CH-5's grammar, to admin chats only.
    /// The log line stays here, because the run log must record what was announced whether or not any
    /// channel was live to carry it.</para>
    ...
    public void NotifyNewOwnerQueueItems(IReadOnlyList<OwnerQueueItem> items)
    {
        ...
        _ = Messenger.PushOwnerQueueAsync(items, DateTime.UtcNow);
    }
