...
public sealed class CourierTask
{
    ...
    /// <summary>PK2.1 / D2(b) - the task's last run time and last result, from
    /// <c>schtasks /Query /V /FO CSV /NH</c>. Null with the scheduler's own words when it cannot say
    /// (no such task, no scheduler).</summary>
    public async Task<(CourierTaskRun? Run, string? Unread)> LastRunAsync()
    {
        var query = await _run(Schtasks, $"/Query /TN \"{Name}\" /V /FO CSV /NH").ConfigureAwait(false);
        if (!query.Ok) return (null, query.Complaint());
        return VerboseLastRun(query.StdOut) is { } run
            ? (run, null)
            : (null, "no last-run columns in the scheduler's answer");
    }

    /// <summary>Columns six and seven of the verbose CSV - Last Run Time and Last Result - by POSITION,
    /// as <see cref="CsvStatus"/> reads the status. Measured 2026-09-17 on the owner's machine:
    /// <c>"HOST","\Conductor Courier","N/A","Running","Interactive only","17/09/2026 10:30:42","267009",...</c>.
    /// The time is in the machine's locale and kept as the scheduler wrote it; the result is a number
    /// in every locale. A task with several triggers prints one row per trigger with the same values,
    /// so the first row is the answer.</summary>
    internal static CourierTaskRun? VerboseLastRun(string csv)
    {
        foreach (var line in csv.Split('\n', StringSplitOptions.RemoveEmptyEntries))
        {
            var fields = SplitCsv(line.Trim());
            if (fields.Count >= 7 && long.TryParse(fields[6], NumberStyles.Integer, CultureInfo.InvariantCulture, out var result))
                return new CourierTaskRun(fields[5], result);
        }
        return null;
    }
