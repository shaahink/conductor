            ...
            "tools/list" => new JsonRpcResponse
            {
                ...
                Result = JsonSerializer.SerializeToElement(new
                {
                    tools = new object[]
                    {
                        ...
                        new { name = "task_update", description = "Update a sub-task's status (todo / in_progress / done / skipped)", inputSchema = new { type = "object", properties = new { taskId = new { type = "string" }, status = new { type = "string" }, tell = new { type = "string", description = "PK4.2: with status done, \"<title> | <two to four sentences>\" for the room. The engine posts the card when the verdict confirms the claim." } }, required = new[] { "taskId", "status" } } },
                        ...
                        new { name = "task_blocked_until", description = "SC5.1: Declare that this session cannot proceed until a known future instant (an external rate-limit window, a scheduled deploy slot). The run loop sleeps until then and spawns ONE more session — no attempt is burned and no fix session is queued. Use this instead of ending a session that only re-derives the same timestamp. End your session immediately after calling it.", inputSchema = new { type = "object", properties = new { until = new { type = "string", description = "The UTC instant the block lifts, ISO 8601 (e.g. 2026-07-31T15:12:00Z). Must be in the future and within 24h." }, reason = new { type = "string", description = "What the run is waiting for. Required — the session that wakes up reads it instead of re-deriving it." } }, required = new[] { "until", "reason" } } },
    ...
    private JsonRpcResponse HandleToolCall(JsonElement id, JsonElement? @params)
    {
        ...
        try
        {
            var payload = name switch
            {
                ...
                "task_blocked_until" => HandleBlockedUntil(args),
