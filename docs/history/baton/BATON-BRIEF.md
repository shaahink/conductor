...
> then your stage file in `docs/history/baton/stages/`. The stage files own the phased tasks and
...
- **Analyzers:** `Meziantou.Analyzer` (https://github.com/meziantou/Meziantou.Analyzer) +
  ...
  analyzer severities (see `docs/history/baton/tooling/` drafts). A curated rule set (not the raw firehose)
...
1. Read `CONDUCTOR-START.md` handoff + this brief + your stage file in `docs/history/baton/stages/`.
...
See `docs/history/baton/stages/B0.md` … `B12.md` for per-stage requirements, task breakdown, session
