...
building-blocks). Read order: this file (handoff + rows) → `docs/history/CONDUCTOR-PLANNER.md` (design brief) →
...
on `ControlAction.ReloadPlan` (`docs/history/CONDUCTOR-AI-NATIVE.md` §G3). Without it every edit here degrades to
...
| P5 | Session-token rollover + limits surfaced honestly in the Face (labeled OFF by default), editable with a session-scoped this-run override; extend ApplyPlanEdit; ride G3 reload. | DONE | see git | Engine: `RunState.MaxSessionTokensThisRun` (null=plan decides · 0=forced OFF this run · >0=this-run cap; run state, evaporates at run end) + the 13th control verb `set-rollover` (tokens/off/clear — all 3 ingresses via ControlFile.Parse; applies immediately, even mid-session; NEVER writes the plan) + `RunContext.EffectiveMaxSessionTokens` as the single choke point for the rollover check AND the soft-break watcher (the two knobs can't disagree). `/plan/edit` limits grew `maxsessiontokens` (empty/0 = OFF, the default) + `softbreakratio` (0<r≤1, empty = the 0.8 default); PlanLimitsDto carries both (absent = OFF on the wire). Face: Settings rows `sessionRollover` (honest browse label: "OFF — DeepSeek-style session rollover disabled (default)" / "ON — roll over past N tokens/session"), `softBreakRatio` ("active only when rollover is ON"), and `rollover (run)` — a control-target row that posts the set-rollover verb, not a plan edit (planField gained a Display hook for browse-view labels); demo mirror; plan_settings golden. Proofs: P5RolloverTests (verb parse; ParseRolloverValue tokens/off/clear/invalid theory; dispatcher writes run state only + plan untouched; wire round-trip incl. OFF-is-absent on GET /plan and invalid-value rejections) + Face round-trips (rollover set/clear; this-run row posts MsgControlSent and PlanVersion does NOT bump) + live proof P5RolloverLiveTests: real run, no cap → session 1 ends normally; `set-rollover 10` (fake agent emits 15 tokens) → next session ends RolledOver, AttemptsThisStage still 0 (no attempt burned), plan file byte-identical. Suite 841 green. |
...
_**P5 DONE (same session) — THE P-SERIES IS CLOSED (P0–P5 all DONE, 2026-07-16).** The rollover is
honestly surfaced: OFF by default everywhere (absent in the plan, absent on the wire, labeled OFF in
the Face Settings), editable as `limits.maxSessionTokens`/`softBreakRatio` riding the G3 live
reload, and flippable for THIS RUN ONLY via the new `set-rollover` control verb (tokens · off ·
clear) — run-state only, proven live to roll a session over with no attempt burned and the plan
file byte-identical. Every rollover/soft-break read goes through `RunContext.EffectiveMaxSessionTokens`._

_**The tracker's mission is delivered.** The planner is a real library (`Conductor.Planning`) with
one-way dependency (arch-enforced), pure decision seams (`IWorkflowResolver` + `IAssignmentPolicy` +
`IQaPolicy` + `PromptComposer`), a declarative `pipeline` rules block, a standalone consumer
(`tools/plan-lint`), live-editable rules (QA dial, limits, rollover, per-task prompt blocks) riding
`reload-plan`, and a Kanban card detail that edits structured task data with advisor-refine. Follow-up
