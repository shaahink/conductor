...
| `task --list` · `task --done <id> --evidence <path> [--tell "<title> \| <two to four sentences>"]` | Checkpoint status from `run.db`; `--done` is a session's claim. `--tell` hands the room's words to the claim: the engine composes the checkpoint card and posts it to the room's observer chat **when the verdict confirms the claim** — a claim whose gates go red posts nothing, and the next session is told the words are held. A session never posts a card itself. |
...
| `courier install [--task-name N] [--exe PATH] [--no-start]` · `courier restart` · `courier stop` · `courier uninstall` | **The lifecycle — set it up once.** `install` registers a per-user Windows Scheduled Task on `conductor-courier.exe`, the courier's own binary: logon trigger, restart-on-failure, and a **keep-alive trigger every five minutes** (`IgnoreNew`) that starts a courier which exited for any reason, exit code 0 included; no admin rights. Nothing else starts the daemon at boot, so a courier you only ever `run` by hand is a courier that stops answering when you close the terminal. `restart` is the fix named by every staleness refusal: the courier is built to outlive every run, so left alone it will happily keep running the binary it started with. `tools/install.ps1` publishes the engine **without stopping it** — the courier runs from `<install>\courier\`, not from the engine's files — and `tools/install.ps1 -CourierOnly` replaces the courier binary alone, re-registers the task and restarts it, never touching `conductor.exe`. |
| `inbox list [--all\|--unseen] [--full] [--json]` · `inbox show --id N` · `inbox add --file PATH [--text T]` · `inbox transcribe (--id N\|--all)` · `inbox parked` · `inbox prune (--id N\|--seen\|--older-than D) [--yes]` | This project's inbox of arrived notes (`.conductor/inbox`, never committed). `list` is the whole inbox with each note's sender (`from`) and Telegram message id (`msg`) — `-` for a note filed before the courier kept them — `show` one note in full with its media and transcript paths, `add` files an audio file or a line of text by hand, `transcribe` runs the `courier` command over a note that arrived without words. `parked` lists the machine-level dead-letter box — notes the courier could not route because the project had moved or vanished; nothing is dropped, it waits there. `prune` is the ONLY thing in conductor that deletes a note — and it will not run without being told which ones. |
| `courier status [--json]` · `courier run [--once]` · `courier allow --repo PATH [--plan NAME]` · `courier deny --repo PATH` · `courier chat --id ID [--profile admin\|observer]` · `courier unchat --id ID` | The machine-level daemon: one bot, always awake, outliving the run. `run` starts `conductor-courier.exe`, its own executable, and waits on it. It owns `CONDUCTOR_TELEGRAM_TOKEN` and is the ONE `getUpdates` consumer this machine has, so notes arrive with nothing running. `status` leads with the heartbeat — `alive`, `stale (last poll N min ago)`, `dead (last seen T)` or `absent` — and a live run starts the task of a dead or stale courier at its next session boundary, saying `courier restarted by this run, Nth time` in its log and on the owner queue (reading a death out: §3). It files only into projects on its **explicit allowlist** — never the whole state catalogue; a live run adds its own project at its first boundary (`POST /hello`, marked `"by": "run <id>"`) — and parks anything it cannot route. With no run live it still answers `/status`, `/progress`, `/money`, `/tokens` and `/evidence` for the chat's project, from that project's `run.db` opened read-only. Telegram keeps an undelivered message for **24 hours**: the courier answers "no run live", not "machine off". A filed note is acknowledged by a reaction on it, never a message; reply `/note` to it for where it went and a **Promote to followup** button: one tap turns that note into a `followups.md` row and a Tier-B fix lane. A note is never auto-injected into a running session - that tier stays a deliberate verb. See [cli.md](cli.md#the-courier--one-bot-always-awake). |
| `say [--to admin\|observer\|ID] (--text T\|--file PATH) [--photo a,b] [--document a,b] [--reply-to ID] [--parse-mode M] [--dry-run]` · `say --react E --message ID` · `say --delete ID` | **New in `v0.6.0`.** One verb for everything sent to a chat. Through the courier when one takes it (the answer carries the message ids, and the courier writes them to `messages.jsonl`); **directly** with `CONDUCTOR_TELEGRAM_TOKEN` when none does, printing `courier unreachable - sent directly`. A live run falls back the same way for its own pushes. Telegram's ceilings (4096 / 1024 / ten files / 10 MB photo) are refused by name before anything is sent; `--dry-run` prints the exact bytes, the resolved chat and the path. See [cli.md](cli.md#say--one-verb-for-everything-sent-to-a-chat). |
| `room list` · `room show [--repo PATH\|--project NAME]` · `room add --repo PATH [--project NAME] [--admin ID] [--observer ID] [--voice PATH] [--counters T] [--footer-live T] [--footer-pending T]` · `room import [--from DIR]` | **New in `v0.6.0`.** The rooms this machine speaks in: a project's admin and observer chats, its card's footer strings and the path to its voice file, kept in the courier home as `rooms/<slug>.json` and never in a repo. `list` prints names only; `show` says whether each chat is set, never the id; `import` is the one-time move from `~/.claude/telegram/*/config.json`, pointing at each `voice.md` rather than copying it, and never overwriting. A repo with no room falls back to its plan's `telegram.chats`, then `courier chat`. See [cli.md](cli.md#rooms--where-a-project-speaks-kept-off-the-repo). |
...
running reached nobody: no error, no queue, no record. The courier is a **separate executable**
(`conductor-courier.exe`, installed to `<install>\courier\`) that owns the token and polls whether
...
conductor courier install                              # the scheduled task: logon, restart-on-failure, keep-alive every 5 min
conductor courier status                               # alive / stale / dead, what version, what offset
```
...
- **Reinstalling the engine leaves the courier running.** It runs its own binary from its own
  directory, so `tools/install.ps1` publishes the engine around it. The courier is replaced with
  `tools/install.ps1 -CourierOnly` (binary, task, restart; `conductor.exe` untouched), which is safe
  with a run live. A courier never restarted keeps running last month's build for weeks; if a run
  refuses a stale courier by name (it names the protocol it speaks and the engine it is still
  running), `conductor courier restart` is the whole fix. A courier newer than the run is not an
  error: protocol 3 still accepts a protocol-2 run's `/push`.
...
### The courier died — reading out why

A dead courier is a quiet inbox, and a quiet inbox looks exactly like a quiet room. Four instruments
record a death (D2). This is how to read one out, the same way every time. Everything here is
**read-only**: do not restart the courier before you have read it, because a start overwrites what the
scheduler remembers about the last run.

**The instruments**

| Instrument | Where | What it says |
|---|---|---|
...
1. **Fix the window in UTC.** `T0` is the last moment the courier is known to be alive (an arming, a
   `courier status` that read `alive`). `T1` is now. Write both down: the finding is dated by them.
2. **`conductor courier status`.** Note the vitals line, the current pid and the engine it runs.
3. **List every start in the window.** Read every `courier run starting` line between `T0` and `T1` in
   `courier.log` and `courier.log.1`. The same pid throughout, with the heartbeat `alive`, means **no
   death occurred**: record that with the instruments above and both timestamps. Otherwise the first
   pid that changed is the death to read out.
4. **Read the predecessor's last lines.** Take the lines just before its successor's `courier run
   starting`, and the successor's death record. Classify:
   ...
   - **A death record and no exit journal at all**: it was killed from outside (`Stop-Process`,
     `taskkill /F`, a crash in native code) or the power went. Compare the record's `last poll T` and
     `task last-run result` with the System log. A `Kernel-Power` 42 just after the last poll, with
     nothing journaled, means **the machine slept**. The record says so, and the keep-alive trigger
     is still the fix.
5. **Check that the supervision worked.** The gap between the predecessor's last poll and the
   successor's start should be about five minutes (the keep-alive trigger) or less (a run's boundary
   restart). A longer gap means neither supervisor ran: a disabled task, the machine off, or no run
   live and the trigger missing. `schtasks /query /tn "Conductor Courier" /xml` shows the trigger.
...
deliberate act — the **Promote to followup** button on the courier's answer to `/note`, or `conductor inbox` and
