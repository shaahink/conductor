...
| `release preflight [--tag 0.6.0] [--base master] [--branch feat/x] [--repo owner/name]` | **New since `v0.5.0`; not in the released binary yet — the era-close, measured instead of written.** Closing an era used to mean a session wrote a runbook and a person performed seven acts by hand from prose; the best one this project produced still let six of its seven acts go unperformed an era earlier with nothing saying so. Six preconditions, one named verdict each, nothing read off a document: **merge** (is `--ff-only` a fast-forward — of the local base *and* of `origin/<base>`, which are different questions, plus a clean tree), **changelog** (`tools/changelog-section.sh` run for real, the same script `release.yml` runs as the first job of a tag build — and a section that exists but says "nothing yet" is red, because that body *is* the release notes), **processes** (is a binary swap safe — the same `UpdateSafety` detector `update` refuses on, with `CONDUCTOR_PID` named among the live engines), **migration** (schema skew: which migration files landed since the commit the *installed* engine was built from), **courier** (would it survive the reinstall — token **persisted** where a logon-triggered task can see it, chats, allowlist, scheduler state; it never dials Telegram, because one `getUpdates` consumer owns the token), **backfill** (which finished run has no GitHub record, by full run id). Writes nothing, merges nothing, tags nothing, installs nothing. Exit **0** all green · **1** something is red · **2** nothing red but something is a judgement only the owner makes — the version number and whether a run joins the published corpus are named and stopped at, never silently skipped. |
| `release perform [--tag 0.6.0] [--yes] [--base master] [--branch feat/x] [--history docs/history]` | **New since `v0.5.0`; not in the released binary yet — the other half of the same idea.** `preflight` measures; `perform` acts on what it measured, and **refuses what is judgement by name rather than skipping it in silence** — the failure that lost six of KS12.3's seven acts, because in prose "this one is yours" and "nobody did this one" are the same sentence. Four **mechanical** acts, each landing as its own commit so the era-close is reviewable one act at a time and each safe to ask twice: the CHANGELOG rename (refused over a placeholder body — that body *is* the release notes), the `--ff-only` merge (gated on `preflight`'s own merge verdict, never a second opinion), the annotated tag (refused until the section it will publish exists; an existing tag is never moved), and the doc move **with** the plan's `tracker`, `planDoc` and `readOrder` repointed *in the same act* — a move without the repoint means the next session reads nothing. Five **owner** acts printed on every run, each with the command: the version number, single-vs-split, whether a run joins the published corpus, the reinstall, and the push. **Dry run is the default**; `--yes` performs. Everything it does is local — nothing is pushed and no binary is swapped — and it refuses outright while a run is live in the plan's own state directory. Exit **0** nothing left to do · **1** an act refused or failed (and nothing after it was attempted) · **2** the mechanical acts are done and the owner's are not, which is what a *finished* run looks like. |
| `release runbook [--tag 0.6.0] [--base master] [--branch feat/x] [--out PATH]` | **New since `v0.5.0`; not in the released binary yet - the owner runbook, generated rather than written.** `.conductor/evidence/KS12/ks12-3-owner-runbook.md` and its DV7 successor were hand-written a fortnight apart in the same shape, and the second one's first finding was that the first had not been carried out - six of its seven acts unperformed, for an era, unnoticed. A document cannot notice an act it never mentioned. This verb calls **the same two entry points** the other two do - the six probes `preflight` runs and the planner `perform` uses in dry run - and renders markdown: the measured preconditions with their verdicts and detail, the four mechanical acts with what each would do right now, the five owner acts each with the command to type, and the order. It **performs nothing**, and unlike `perform` it does **not** refuse a live run - it only measures, and mid-run is when the close is most worth reading. Rendering is pure and the timestamp is an input, so the same facts render byte-identical output and a regenerated runbook diffs to exactly what moved. stdout by default; `--out` writes the file and says where. Exits with `preflight`'s code. |
