...
Every verb that interrupts a live run takes the same two confirmations, so they are written here
once rather than on ten rows: `--yes` confirms the destructive act, and `--force` proceeds despite a
condition the verb would otherwise refuse on. They are declared by `abort`, `kill`, `pause`,
`pause-after-stage`, `resume`, `retry-stage`, `skip`, `heartbeat`, `rollback`, `plan` and `update`.
The `rollback` row below says what each means there, and the two are not synonyms anywhere.
...
| `log` | Query the structured JSON log: `-q`/`--query "stage=P7 and gate=build and outcome=fail"` — key=value pairs joined by ` and `, case-insensitive. `--tail <N>` shows only the last N matches, and `--since <D>` bounds how far back it reads. |
| `tasks` | Sub-task graph per checkpoint from the event log. |
| `task` | Checkpoint CRUD from run.db: `--list`, `--done`, `--in-progress`. **This is the one claim path** — hand-editing a tracker row claims nothing. A claim carries its receipts: `-c`/`--commit <SHA>` and `-e`/`--evidence <TEXT>`, and, **new since `v0.5.0`**, `--tell "<title> | <two to four sentences>"` - the words for the room. They are stored on the claim and the **engine** posts the checkpoint card to the room's observer chat when the verdict confirms the claim; a claim whose gates go red posts nothing, and the next session's prompt says the words are held. Malformed words (no `|`, a title over 120 characters, sentences over 700) are refused at the claim. The MCP `task_update` tool takes the same words as `tell`. The other moves put a card back or park it: `--todo <CP>` reopens a done/skipped/blocked card, `--blocked <CP>` marks work that cannot proceed, `--skipped <CP>` marks work deliberately not delivered, and `--blocked-until <ISO8601>` (with `--reason`) is the *timed* wait, which is a different thing — conductor sleeps until that instant and spawns one more session. `--amend <CP> --note "<TEXT>"` records an acceptance correction on a card without moving it. Every move prints the card's post-fold status and exits non-zero if the transition is refused, which is why the output is trustworthy and the intent is not. |
| `note` / `bug` | Knowledge ledger + tracked bugs that outlive the session that found them. `note` takes `-k`/`--kind <finding\|observation\|trap\|decision>` (default `note`) and `-s`/`--stage <STAGE>`; a text of `-` reads the body from stdin (`conductor note - < body.md`). **Use stdin for anything multi-line** (bug #75): the installed binary is reached through a `.cmd` shim, and `cmd.exe` ends an argument at its first newline, so a multi-line argument files as its first line alone — the verb warns when a body ends in a colon with nothing after it. `bug new` takes `-d`/`--detail <TEXT>` for the repro (or `--detail -` to read it from stdin), `-s`/`--severity <low\|medium\|high>` (default medium) and `--stage <STAGE>`; `bug list` prints the open ones (`--all` includes the closed), and `bug fix <id> --wontfix` closes one as wontfix rather than fixed. |
...
| `worktree` | What attempt worktrees conductor has on disk: which run made each, which are orphans from a run that died, and which belong to a live run. Read-only; `--json` for machines, and `--reap` removes the orphans - it never touches a worktree you made or a live run's. The engine runs the same sweep at startup, so this verb is mostly for the one case that survives it: a tree whose build output is still locked by a process that outlived the run. |
...
| `release preflight [--tag 0.6.0] [--base master] [--branch feat/x] [--repo owner/name] [--courier-task <NAME>]` | **New in `v0.6.0` — the era-close, measured instead of written.** Closing an era used to mean a session wrote a runbook and a person performed seven acts by hand from prose; the best one this project produced still let six of its seven acts go unperformed an era earlier with nothing saying so. Seven preconditions, one named verdict each, nothing read off a document: **merge** (is `--ff-only` a fast-forward — of the local base *and* of `origin/<base>`, which are different questions, plus a clean tree), **changelog** (`tools/changelog-section.sh` run for real, the same script `release.yml` runs as the first job of a tag build — and a section that exists but says "nothing yet" is red, because that body *is* the release notes), **docs** (every `docs/*.md` row still saying "not in the released binary yet" — false the instant the tag lands; the `docs` act below rewrites them to name the release, bug #95), **processes** (is a binary swap safe — the same `UpdateSafety` detector `update` refuses on, with `CONDUCTOR_PID` named among the live engines), **migration** (schema skew: which migration files landed since the commit the *installed* engine was built from), **courier** (would it survive the reinstall — token **persisted** where a logon-triggered task can see it, chats, allowlist, scheduler state, and where the live courier runs from: in its own directory the engine installs around it, still inside or beside the engine it is moved once; `--courier-task` reads a rig's scratch task instead of yours; it never dials Telegram, because one `getUpdates` consumer owns the token), **backfill** (which finished run has no GitHub record, by full run id). Writes nothing, merges nothing, tags nothing, installs nothing. Exit **0** all green · **1** something is red · **2** nothing red but something is a judgement only the owner makes — the version number and whether a run joins the published corpus are named and stopped at, never silently skipped. |
| `release perform [--tag 0.6.0] [--yes] [--base master] [--branch feat/x] [--history docs/history]` | **New in `v0.6.0` — the other half of the same idea.** `preflight` measures; `perform` acts on what it measured, and **refuses what is judgement by name rather than skipping it in silence** — the failure that lost six of KS12.3's seven acts, because in prose "this one is yours" and "nobody did this one" are the same sentence. Five **mechanical** acts, each landing as its own commit so the era-close is reviewable one act at a time and each safe to ask twice: the CHANGELOG rename (refused over a placeholder body — that body *is* the release notes), the docs rows rewritten from "New since `v<prev>`; not in the released binary yet" to "New in `v<tag>`" (bug #95), the `--ff-only` merge (gated on `preflight`'s own merge verdict, never a second opinion), the annotated tag (refused until the section it will publish exists; an existing tag is never moved), and the doc move **with** the plan's `tracker`, `planDoc` and `readOrder` repointed *in the same act* — a move without the repoint means the next session reads nothing. Five **owner** acts printed on every run, each with the command: the version number, single-vs-split, whether a run joins the published corpus, the reinstall, and the push. **Dry run is the default**; `--yes` performs. Everything it does is local — nothing is pushed and no binary is swapped — and it refuses outright while a run is live in the plan's own state directory. Exit **0** nothing left to do · **1** an act refused or failed (and nothing after it was attempted) · **2** the mechanical acts are done and the owner's are not, which is what a *finished* run looks like. |
| `release runbook [--tag 0.6.0] [--base master] [--branch feat/x] [--out PATH]` | **New in `v0.6.0` - the owner runbook, generated rather than written.** `.conductor/evidence/KS12/ks12-3-owner-runbook.md` and its DV7 successor were hand-written a fortnight apart in the same shape, and the second one's first finding was that the first had not been carried out - six of its seven acts unperformed, for an era, unnoticed. A document cannot notice an act it never mentioned. This verb calls **the same two entry points** the other two do - the seven probes `preflight` runs and the planner `perform` uses in dry run - and renders markdown: the measured preconditions with their verdicts and detail, the five mechanical acts with what each would do right now, the five owner acts each with the command to type, and the order. It **performs nothing**, and unlike `perform` it does **not** refuse a live run - it only measures, and mid-run is when the close is most worth reading. Rendering is pure and the timestamp is an input, so the same facts render byte-identical output and a regenerated runbook diffs to exactly what moved. stdout by default; `--out` writes the file and says where. Exits with `preflight`'s code. |
...
| `money` | Price a run or a project from its own ledger: sessions, tokens, cache-read share, cost, checkpoints, tokens and dollars per checkpoint, plus the windows either side of a cap change, the per-stage split and the calendar month. Scopes: `--run`, `--project`, `--since`, `--plan`, `--repo`, `--json`. |
...
**New in `v0.6.0`.** A **room** is the pair of chats a project
