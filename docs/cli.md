...
| `new-plan` | Bare-minimum scaffold: plan + tracker, no gate detection. `-o|--output <DIR>` and `--name <NAME>` as on `init`. |
...
| `mcp-serve` | Run the MCP task server (JSON-RPC 2.0 over stdio). The engine wires this up itself for a session; the paths exist for driving it by hand — `--events <path>` the events.jsonl, `--journal <path>` the MCP side-journal, `--run-db <path>` the store (default: beside the events file, the pre-K3.1 layout), `--run-id <id>` for event authorship, `--state-dir <path>` for the bg tools, and `--session <number>`, which is stamped on every bg child the server starts. |
