...
| [`NEXT-ERA-FINDINGS-2026-08-23.md`](NEXT-ERA-FINDINGS-2026-08-23.md) | **Findings for the era after Karvansara, not yet a plan.** The feedback inbox (voice notes and files into a per-project record), whether sessions can run on Anthropic's cloud (measured against `--cloud`: no, not the session loop), and the merged ranking of both this document's asks and the observability backlog. Written to be argued with before it becomes `plans/`. |
