...
- **W4.1 — Import carries the work.** `ImportResult` gains checkpoints:
  ...
  `plan import docs/history/MAESTRO-PLAN.md` on a scratch repo → `conductor run` reaches session 1 with
...
- **W5.2 — The real thing (HUMAN: owner starts it; paid).** A real-model (cheap tier),
  ...
  demonstrated and written up as an audit (`docs/dev/workgraph/W5-AUDIT.md`) with run.db/log
...
- **W6.4 — Hygiene + landing.** Archive historical trackers to `docs/history/archive/trackers/`
