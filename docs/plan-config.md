...
| `board` | string | `"issues"` | `issues`, or `issues+project` for a Projects v2 board as well (needs `projectNumber` and a token with the `project` scope). Any other value is refused **by name** rather than read as the default — silently downgrading a typo to issues-only is indistinguishable from a project mirror that ran and did nothing. Without the `project` scope the project half refuses by name and the issue board is unaffected: see [cli.md](cli.md#the-projects-v2-board-and-the-one-scope-it-needs). |
