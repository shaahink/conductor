...
_**NEXT: P4 (finish the decoupling)** — move the remaining workflow/assignment/QA *decision* logic
out of `VerdictEngine.Workflow.cs` behind the P0 seams; engine passes POCO facts in, executes
decisions out. The gate is a standalone consumer (test project or `tools/plan-lint`) referencing
ONLY `Conductor.Planning` that loads a plan file and prints resolved workflow + assignment + QA for
a stage — zero transitive engine dependency; no planning-domain type left in the engine assembly;
no behavior change (existing suite is the guard). Then P5 (rollover surfaced, independent).
Read `docs/CONDUCTOR-PLANNER.md` §P4 before starting._
