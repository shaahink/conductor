...
| The era-close, performed | `Core/Release/ReleasePerform.cs` | Five mechanical acts (`MechanicalOrder`, `:40`; the fifth, `docs`, is bug #95) and five owner acts (`OwnerOrder`, `:68`) as **data**. An owner act is *refused by name*, never skipped — in prose "this one is yours" and "nobody did this one" are the same sentence, and that ambiguity lost six of KS12.3's seven acts for an era |
